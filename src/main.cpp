...
uint256 hashGenesisBlock("0x334388d94fc1b4b075fd0d3b59fa7b3db20ba9cccdca3ea24976baaa38f596c4");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Houseofcoins: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Houseofcoins Signed Message:\n";
...
{
    // Houseofcoins: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 60 * 2;  // Houseofcoins:  Look at last 120 seconds
static const int64 nTargetSpacing = 60 * 2; // Houseofcoins: 2 minutes per  block
static const int64 nInterval = nTargetTimespan / nTargetSpacing;   //---------> [120 / 120 = 1] change every 1 block
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Houseofcoins: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Houseofcoins: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x334388d94fc1b4b075fd0d3b59fa7b3db20ba9cccdca3ea24976baaa38f596c4");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Time flies like an arrow. HouseofCoins.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1397681642;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2468528;
        ...
        assert(block.hashMerkleRoot == uint256("0x6f12091a14f883e83de686773708b9124cb0156e4b2de1f2b1417703492bb7c2"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Houseofcoins: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("HouseofcoinsMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("HouseofcoinsMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static HouseofcoinsMiner(CWallet *pwallet)
{
    printf("HouseofcoinsMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("houseofcoins-miner");
    ...
    try { loop {
        ...
        printf("Running HouseofcoinsMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("HouseofcoinsMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&HouseofcoinsMiner, pwallet));
}
